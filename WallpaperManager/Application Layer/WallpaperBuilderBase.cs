...
namespace WallpaperManager.Application {
  ...
  public abstract class WallpaperBuilderBase {
        ...
        try {
          ...
          for (Int32 i = 0; i < this.ScreensSettings.Count; i++) {
            ...
            WallpaperBuilderBase.DrawOverlayTexts(
              ...
              this.ScreensSettings[i].OverlayTexts
            );
            destinationGraphics.ResetClip();
          }
      ...
      using (Graphics graphics = Graphics.FromImage(wallpaperImage)) {
        ...
        try {
          ...
          for (Int32 i = 0; i < screenCount; i++) {
            ...
            WallpaperBuilderBase.DrawOverlayTexts(
              destinationGraphics, screenBounds, wallpapers, this.ScreensSettings[i].OverlayTexts
            );
            destinationGraphics.ResetClip();
          }
    ...
    /// <summary>
    ///   Draws the given <see cref="WallpaperOverlayText" /> objects by using the given <see cref="Graphics" /> object.
    /// </summary>
    /// <param name="destGraphics">
    ...
    ///   A collection of <see cref="WallpaperOverlayText" /> objects to be drawn.
    /// </param>
    /// <exception cref="ArgumentNullException">
    ...
    private static void DrawOverlayTexts(
      Graphics destGraphics, Rectangle rect, IList<Wallpaper> wallpapers, IList<WallpaperOverlayText> overlayTexts
    ) {
      if (destGraphics == null) {
        throw new ArgumentNullException(ExceptionMessages.GetVariableCanNotBeNull("destGraphics"));
      }
      ...
      try {
        ...
        foreach (WallpaperOverlayText overlayText in overlayTexts) {
          if (overlayText != null) {
            using (StringFormat format = new StringFormat(StringFormatFlags.NoWrap | StringFormatFlags.NoClip)) {
              switch (overlayText.Position) {
                case OverlayTextPosition.TopLeft:
                  format.Alignment = StringAlignment.Near;
                  format.LineAlignment = StringAlignment.Near;
                  break;
                case OverlayTextPosition.TopMiddle:
                  format.Alignment = StringAlignment.Center;
                  format.LineAlignment = StringAlignment.Near;
                  break;
                case OverlayTextPosition.TopRight:
                  format.Alignment = StringAlignment.Far;
                  format.LineAlignment = StringAlignment.Near;
                  break;
                case OverlayTextPosition.BottomLeft:
                  format.Alignment = StringAlignment.Near;
                  format.LineAlignment = StringAlignment.Far;
                  break;
                case OverlayTextPosition.BottomMiddle:
                  format.Alignment = StringAlignment.Center;
                  format.LineAlignment = StringAlignment.Far;
                  break;
                default:
